...
import { Patient,initialPatient } from "src/types/patients";
...
//     useAccountContext();
  ...
  const handleSubmitHelper = useFunction(handleSubmit, {
    successMessage: patient ? "Cập nhật thành công!" : "Thêm thành công!",
  });

  const formik = useFormik<Patient>({
    initialValues: patient || initialPatient,
    onSubmit: async (values) => {
        const { error } = await handleSubmitHelper.call(values);
        if (!error) {
          formik.setValues(initialPatient);
          onClose();
        }
        console.log(values);
    },
            ...
            >
              <Box>
                ...
                <Typography variant="h6">
                  {patient ? "chỉnh sửa bệnh nhân" : "Thêm tài khoản mới"}
                </Typography>
              </Box>
